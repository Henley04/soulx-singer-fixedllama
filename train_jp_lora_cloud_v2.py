...
与 train_jp_lora_cloud.py (v1) 的差异：
  ...
  4. 训练目标：embedding 只训练 jp_cl/jp_n 两行；preflow LoRA + cond_emb 保持 v1 方案
     （因音素组合变化，跨模态对齐层仍需微调）。
  5. GRAD_CHECKPOINT：建议关闭（v1 已改 False）。diff_estimator 虽冻结，但
     cond_emb 可训练时梯度需穿过它回传，checkpointing 能省激活显存；但
     A100 40G 显存充裕，重计算是纯速度开销。

直接运行：python train_jp_lora_cloud_v2.py
  - 无参数：全流水线（prepare → train → export）
  - --prepare_only / --train_only / --export_only：分阶段执行
  - --demo：仅查看音素映射和强制对齐演示（不需模型/数据）
  - 其他参数同 train_jp_lora_cloud.py（--no_jsut / --device 等）
实现方式：运行时 monkey-patch v1 模块的关键函数为 v2 版本，复用 v1 全部基础设施。
"""
...
# ===========================================================================
# 8. 自包含入口：直接 python train_jp_lora_cloud_v2.py 即可启动训练
# ===========================================================================
# 策略：import v1 模块，用 monkey-patch 把 v1 的关键函数/常量替换为 v2 版本，
# 然后调用 v1 的 main()。所有数据准备、训练循环、导出逻辑复用 v1。
# v2 只覆盖差异点：音素映射、embedding 初始化、trainable 设置、优化器、强制对齐。

def _patch_v1_with_v2():
    """把 v1 模块的关键符号替换为 v2 版本。返回 v1 模块对象。"""
    import train_jp_lora_cloud as v1

    # --- 替换 STAGE_CONFIGS ---
    v1.STAGE_CONFIGS = STAGE_CONFIGS_V2

    # --- 替换 init_jp_embeddings ---
    # v1 签名: init_jp_embeddings(model, mapping_path, phoneset_path)
    # v2 签名: init_jp_embeddings_v2(model, phoneset_path)  -- 忽略 mapping_path
    def init_jp_embeddings_wrapper(model, mapping_path=None, phoneset_path=None):
        return init_jp_embeddings_v2(model, phoneset_path or v1.PHONE_SET_PATH)
    ...
    # v2 签名: apply_lora_and_setup_trainable_v2(model, stage, phoneset_path)
    def apply_lora_wrapper(model, stage):
        return apply_lora_and_setup_trainable_v2(model, stage, v1.PHONE_SET_PATH)
    v1.apply_lora_and_setup_trainable = apply_lora_wrapper

    # --- 替换 build_optimizer ---
    # 签名一致：(model, stage, lora_lr, embed_lr)
    v1.build_optimizer = build_optimizer_v2

    # --- 替换 lab_phonemes_to_notes ---
    # v1 签名: lab_phonemes_to_notes(lab_segments, midi_notes, sample_rate=24000, hop_size=480)
    # v2 签名一致
    v1.lab_phonemes_to_notes = lab_phonemes_to_notes_v2

    # --- 替换 _gtsinger_word_to_tokens ---
    # v1 签名: _gtsinger_word_to_tokens(word_entry, min_dur=0.12)
    # v2 签名: gtsinger_word_to_tokens_v2(word_entry)  -- 忽略 min_dur（内部硬编码）
...
def main():
    """v2 主入口：--demo 跑逻辑演示，否则 patch v1 后启动完整训练流水线。"""
    # 检查 --demo flag（不消费 sys.argv，让 v1 argparse 忽略它）
    if '--demo' in sys.argv:
        _demo()
        return

    print('=' * 60)
    print('train_jp_lora_cloud_v2.py — 促音/拨音独立训练 + 复用音素冻结')
    print('=' * 60)
    print('策略：只训练 jp_cl/jp_n 两个新 token，其余 31 个日语音素')
    print('      近似映射到英语音素完全复用（冻结），新增强制对齐。')
    print()

    v1 = _patch_v1_with_v2()
    v1.main()
